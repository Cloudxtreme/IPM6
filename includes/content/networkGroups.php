...
elseif ($allNetworks->_networks_total > 0) {
	...
	do { 
		...
		$network = new network($row_networks['id'], $db);
	...
	?>
		...
		<div id="utilization<?php echo $utilization; ?>"><?php if (!$network->_network_subnetted && isnull($network->_network_v6_mask) && $network->_network_mask == ip2long("255.255.255.254")) { printf("%d%%", $network->_network_addresses / 2 * 100); } elseif (!$network->_network_subnetted && isnull($network->_network_v6_mask) && $network->_network_mask != ip2long("255.255.255.255")) { printf("%d%%", $network->_network_addresses / $net['total'] * 100); } else { echo "N/A"; } ?></div><div id="network"><a href="" title="Browse this network" onclick="ShowContent('networks',<?php echo $_GET['container']; ?>,<?php echo $row_networks['id']; ?><?php if (!isnull($_GET['group'])) { echo ",".$_GET['group']; } ?>); return false;"><?php if ($row_networks['v6mask']) { echo Net_IPv6::Compress(long2ipv6($row_networks['network']))."/".$row_networks['v6mask']; } else { echo long2ip($row_networks['network']).get_slash(long2ip($row_networks['maskLong'])); } ?></a>&nbsp; &nbsp;&nbsp;<?php echo $row_networks['descr']; ?></div>
